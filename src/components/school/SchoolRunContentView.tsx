...
export default function SchoolRunContent() {
  ...
  useEffect(()=>{if(!isLoading&&isAuthenticated&&geo.state.status!=='locating') load();else if(!isLoading&&geo.state.status!=='locating')setLoading(false);},[isLoading,isAuthenticated,shortlist.join('|'),geo.state.status,geo.state.lat,geo.state.lng]);
