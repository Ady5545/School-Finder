...
import { Compass, Scale, Calendar, Menu, Search, User, Heart } from 'lucide-react';
...
export const Header: React.FC = () => {
  ...
  return (
    <header className="sticky top-0 z-40 w-full bg-white/95 backdrop-blur-md border-b border-[var(--color-border)] shadow-xs">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between gap-4">
        ...
        <div className="hidden sm:flex items-center gap-2.5">
          ...
          <Link href="/auth/login">
            <Button variant="ghost" size="sm" leftIcon={<User className="w-3.5 h-3.5" />} className="text-xs">
              Log in
            </Button>
          </Link>
        </div>
        ...
        <div className="flex items-center gap-1.5 sm:hidden">
          ...
          <Link href="/wishlist" aria-label={`Shortlist (${shortlist.length})`}>
            ...
            >
              ...
              {shortlist.length > 0 && (
                <span className="absolute 1 top-0.5 right-0.5 w-3.5 h-3.5 bg-rose-500 text-white rounded-full text-[8px] font-bold flex items-center justify-center">
                  {shortlist.length}
                </span>
              )}
            </IconButton>
      ...
      >
        <div className="flex flex-col gap-6">
          ...
          <div className="mt-auto pt-6 text-xs text-[var(--color-content-muted)] border-t border-[var(--color-border-subtle)]">
            <p className="font-bold text-[var(--color-content)]">Greater Noida West & Extension</p>
            <p className="mt-1 leading-relaxed">
              Parent-first intelligence, verified fee breakdowns, and unbiased school comparisons.
            </p>
          </div>
        </div>
      </Drawer>
