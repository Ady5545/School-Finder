...
export async function GET(req: NextRequest) {
  ...
  const schoolMetrics = schools.map(school => {
    ...
    return {
      ...
      board: Array.isArray(school.board) ? school.board.join(', ') : school.board || 'Not set',
      ...
      verifiedFee: school.fees?.annualDisplay || school.fees?.rangeText || school.fees?.tuitionAnnual || (school.fees?.cardFee ? `₹${school.fees.cardFee.toLocaleString('en-IN')}/yr` : 'Not set'),
