...
export function createAdminSchool(
  ...
  const newSchool: School = {
    ...
    curriculum: schoolData.curriculum || (Array.isArray(schoolData.board) ? schoolData.board.join(', ') : ''),
    ...
    location: {
      ...
      coordinates: {
        lat: schoolData.location?.coordinates?.lat ?? schoolData.location?.coordinates?.latitude ?? null,
        lng: schoolData.location?.coordinates?.lng ?? schoolData.location?.coordinates?.longitude ?? null,
        latitude: schoolData.location?.coordinates?.latitude ?? schoolData.location?.coordinates?.lat ?? null,
        longitude: schoolData.location?.coordinates?.longitude ?? schoolData.location?.coordinates?.lng ?? null,
        isVerified: Boolean(schoolData.location?.coordinates?.isVerified),
      },
    },
    fees: {
      ...(schoolData.fees || {}),
    ...
    verification: {
      ...(schoolData.verification || {}),
      isVerified: Boolean(schoolData.verification?.isVerified),
      status: schoolData.verification?.status || 'pending_audit',
      lastVerified: schoolData.verification?.lastVerified || new Date().toISOString().split('T')[0],
      sourceName: schoolData.verification?.sourceName || 'Admission Pitara Admin CMS',
      cbseAffiliationNumber: schoolData.affiliationNumber || schoolData.verification?.cbseAffiliationNumber || null,
      verifiedFields: schoolData.verification?.verifiedFields || [],
    },
    legacyIdentifiers: {
    ...
    classification: schoolData.classification || 'core_greater_noida_west',
    status: schoolData.status || 'active',
    isArchived: Boolean(schoolData.isArchived),
    ...
    isDuplicate: Boolean(schoolData.isDuplicate),
    duplicateOf: schoolData.duplicateOf || null,
    affiliationNumber: schoolData.affiliationNumber || schoolData.verification?.cbseAffiliationNumber || null,
    establishedYear: schoolData.establishedYear || null,
    sports: schoolData.sports || [],
    timings: schoolData.timings || {},
    transportNotes: schoolData.transportNotes || '',
    editorialNotes: schoolData.editorialNotes || '',
  };
